	...
	private ViewUserUpdate() {
		...
		button_UpdatePassword.setOnAction((event) -> {
			...
			result.ifPresent(newPw -> {
				// Validate password
				String passwordError = Model.evaluatePassword(newPw);
				if (passwordError.length() > 0) {
					Alert a = new Alert(AlertType.ERROR);
					a.setTitle("Invalid Password");
					a.setHeaderText("Please correct your Password");
					a.setContentText(passwordError);
					a.showAndWait();
					return;
				}

				// Must be different from the current password
				String currentPw = theUser.getPassword();
				if (currentPw.compareTo(newPw) == 0) {
					String msg = "*** ERROR *** The new password must be different from the current password!";
					Alert a = new Alert(Alert.AlertType.ERROR);
					a.setTitle("Invalid Password");
					a.setHeaderText("Password not changed");
				...
				String updatedPw = theDatabase.getCurrentPassword();
				theUser.setPassword(updatedPw);
				label_CurrentPassword.setText(updatedPw);
			});
		});
