...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		String usernameError = ModelFirstAdmin.checkForValidUserName(username);
		if (usernameError.length() > 0) {
			System.err.println(usernameError);
			Alert a = new Alert(AlertType.ERROR);
			a.setTitle("Invalid Username");
			a.setHeaderText("Please correct the Username");
			a.setContentText(usernameError);
			a.showAndWait();
			return;
		}

		// Make sure the password is valid
		String passwordError = ModelFirstAdmin.evaluatePassword(password);
		if (passwordError.length() > 0) {
			System.err.println(passwordError);
			Alert a = new Alert(AlertType.ERROR);
			a.setTitle("Invalid Password");
			a.setHeaderText("Please correct the Password");
